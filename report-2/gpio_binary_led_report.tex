...
Board Used    & Raspberry Pi (BOARD numbering) \quad\textbullet\quad verified on ESP32 \\
\bottomrule
\end{tabularx}
...
7 & ESP32 development board (optional)        & 1 & Cross-checking the same logic \\
\bottomrule
\end{tabularx}
...
\subsection{Cross-check on the ESP32}

To confirm that the result depends on the logic and not on the particular board, the
same experiment was repeated on an ESP32, with the eight LEDs moved to GPIO 23, 22, 21,
19, 18, 5, 4 and 2 (most significant bit first) and the number entered in the Arduino
IDE Serial Monitor instead of a terminal. The conversion is written as a shift-and-mask
loop rather than a format string, but is equivalent:

\begin{lstlisting}[language=C++, caption={The same conversion on the ESP32, reading the Serial Monitor.}, label={lst:esp}]
...
3 & $200$ & \texttt{11001000} & $128+64+8 = 200$ \\
4 & $20$  & \texttt{00010100} & $16+4 = 20$ \\
5 & $25$  & \texttt{00011001} & $16+8+1 = 25$ \\
\bottomrule
\end{tabularx}

\vspace{8pt}
\noindent Five outputs were recorded in total --- two on the Raspberry Pi itself and
three on the ESP32 cross-check. In every case the sum of the place values of the lit
LEDs equals the number that was entered, confirming that the display is a correct
binary representation.

\subsection*{Outputs 1 and 2 --- Raspberry Pi terminal}
...
\subsection*{Outputs 3--5 --- cross-check on the ESP32}

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth, trim=393 102 365 300, clip]{figures/esp32_serial_monitor.jpg}
\caption{Three further outputs, taken on the ESP32 cross-check:
\textbf{Output 3} $200 \rightarrow \texttt{11001000}$, \textbf{Output 4}
$20 \rightarrow \texttt{00010100}$ and \textbf{Output 5}
$25 \rightarrow \texttt{00011001}$. Each printed string matched the pattern of lit LEDs
on the breadboard.}
\label{fig:out234}
\end{figure}
...
hardware. Repeating it on an ESP32 with a different conversion method and a different
input path produced identical behaviour, confirming that the result follows from the
binary representation itself and not from any feature of a particular board.
