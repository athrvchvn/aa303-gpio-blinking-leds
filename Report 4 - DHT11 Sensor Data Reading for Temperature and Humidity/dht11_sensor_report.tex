...
\newcommand{\expdate}{10 September 2026 (Raspberry Pi) and 3 September 2026 (ESP32)}   % <-- CHECK THESE DATES
...
%==============================================================================
% \dhtschematic{board}{vcc pin}{data pin}{gnd pin}
\newcommand{\dhtschematic}[4]{%
\begin{circuitikz}[scale=0.74, transform shape, every node/.style={font=\footnotesize}]
  % ---- controller header
  \draw[fill=accentlt, draw=accent, line width=0.9pt, rounded corners=4pt]
        (-3.6,-0.35) rectangle (-0.6,3.6);
  \node[text=accent, font=\small\bfseries] at (-2.1,3.1) {#1};
  \node[text=cmnt,   font=\scriptsize]     at (-2.1,2.7) {GPIO header};
  \foreach \y/\pin/\name in {2.0/#2/3.3\,V, 1.1/#3/data, 0.2/#4/GND}{
...
{\color{white}
 ...
 \noindent{\small Reading a digital temperature and humidity sensor over a single GPIO pin --- printed on a Raspberry Pi terminal, and served by an ESP32 as a web page}
}
\vspace{1.5cm}
...
Boards Used   & Raspberry Pi 4 (GPIO2, physical pin 3) and ESP32 DevKit (GPIO4) \\
\bottomrule
\end{tabularx}
...
quantities as a digital 40-bit frame over a single data wire. In \textbf{Part A} a
program on the Pi requests a reading every two seconds, decodes the frame, checks it,
and prints the temperature in $^{\circ}\mathrm{C}$ and the humidity in \% to the
terminal. In \textbf{Part B} the same sensor is read by an \textbf{ESP32}, which hosts
its own Wi-Fi network and serves the two values as a web page to a phone.
\end{keybox}
...
\begin{itemize}
  ...
  \item To read the same sensor from an ESP32 with the Arduino DHT library and publish
        the values over Wi-Fi as a web page, so that they can be viewed on a phone.
\end{itemize}
...
$3.3\,\mathrm{V}$, which the DHT11 tolerates. On the ESP32 any free GPIO can be used;
GPIO4 was used here.
\end{notebox}
...
\midrule
1 & Raspberry Pi 4 (in case, with SD card, OS installed) & 1 & Part A host; drives the single-wire bus and prints to the terminal \\
2 & ESP32 DevKit (30-pin)                                  & 1 & Part B host; reads the sensor and serves the web page over Wi-Fi \\
3 & DHT11 sensor module, HW-481 (3-pin, with pull-up)  & 1 & Measures temperature and relative humidity; shared by both parts \\
4 & Female--female jumper wires                           & 3 & Supply, data and ground between the header and the module \\
5 & USB-C power supply, monitor, keyboard and mouse       & 1 & Running and observing the Raspberry Pi \\
6 & Micro-USB cable, laptop with Arduino IDE, and a phone with Wi-Fi & 1 & Programming the ESP32 and viewing its page \\
7 & Software: \texttt{adafruit-circuitpython-dht} + \texttt{libgpiod2} (Pi); \texttt{DHT sensor library} by Adafruit (Arduino) & --- & Libraries that implement the DHT11 protocol \\
\bottomrule
\end{tabularx}
...
board's header with three jumper wires --- supply to $3.3\,\mathrm{V}$, ground to GND,
and the signal pin to one GPIO. \textbf{Part A} used the Raspberry Pi: the Adafruit DHT
library was installed, and a short Python program creates a \texttt{DHT11} object on
\texttt{board.D2}, then loops forever reading \texttt{temperature} and
\texttt{humidity} every two seconds and printing them. Failed reads --- which the
library reports as a \texttt{RuntimeError} when the checksum does not match or the
frame is incomplete --- are caught, printed, and simply retried on the next cycle.
\textbf{Part B} moved the same module to an ESP32: an Arduino sketch reads it with the
Adafruit \texttt{DHT} library, starts the ESP32 as a Wi-Fi access point, and answers
web requests with a page containing the current values, which was opened on a phone.
...
\toprule
\rowcolor{soft}
\textbf{Module pin} & \textbf{Raspberry Pi (Part A)} & \textbf{ESP32 (Part B)} \\
\midrule
$+$ (VCC)    & Pin 1 --- $3.3\,\mathrm{V}$ (red) & 3V3 \\
S (signal)   & Pin 3 --- GPIO2 (purple)          & GPIO4 \\
$-$ (GND)    & Pin 6 --- GND (black)             & GND \\
\bottomrule
\end{tabularx}
\end{minipage}\hfill
...
\centering
\dhtschematic{Raspberry Pi}{Pin 1}{Pin 3}{Pin 6}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\textwidth}
\centering
\dhtschematic{ESP32}{3V3}{GPIO4}{GND}
\end{minipage}
\caption{Circuit schematics for Part A (left) and Part B (right). The HW-481 module
needs only supply, ground and one data line; its on-board $10\,\mathrm{k}\Omega$
resistor holds the data line HIGH when neither side is driving it.}
\label{fig:schematic}
\end{figure}
...
The program itself is Listing~\ref{lst:dht}.
...
\subsection{Part B --- ESP32, readings on a web page}

On the ESP32 the sensor is read with the Arduino \emph{DHT sensor library} by Adafruit
(installed from the IDE's Library Manager), whose \texttt{readTemperature()} and
\texttt{readHumidity()} return \texttt{NaN} when a frame fails its checksum. Rather
than printing to the serial monitor, the sketch (Listing~\ref{lst:esp}) turns the ESP32
into a Wi-Fi \textbf{access point} named \texttt{ESP32\_DHT11} and runs a web server on
it. Every request for \texttt{/} triggers a fresh read of the sensor and returns a
small HTML page with the two values --- or ``Sensor Error!'' if the read failed --- and
the page asks the browser to reload itself every two seconds. A phone joined to the
ESP32's network and opened \texttt{http://192.168.4.1}, the access point's address,
which the sketch also prints on the Serial Monitor at start-up.

\Needspace{18\baselineskip}
\begin{lstlisting}[language=C++, caption={Part B --- ESP32 sketch: the DHT11 readings served as a web page from the board's own access point.}, label={lst:esp}]
#include <WiFi.h>
#include <WebServer.h>
#include <DHT.h>
...
\begin{enumerate}
  \item \textbf{Part A.} With the Raspberry Pi powered off, the module's $+$, S and $-$
        pins were connected with three jumper wires to pin~1 ($3.3\,\mathrm{V}$), pin~3
        (GPIO2) and pin~6 (GND) of the header, and the connections were checked against
        the pinout.
  ...
  \item The program was stopped with \texttt{Ctrl+C}, which releases the sensor and
        the GPIO pin.
  \item \textbf{Part B.} The module was moved to the ESP32 ($+$ to 3V3, $-$ to GND,
        S to GPIO4). In the Arduino IDE the ESP32 board package and the Adafruit DHT
        library were installed, Listing~\ref{lst:esp} was uploaded to the board on its
        COM port, and the Serial Monitor showed the network name and the address
        \texttt{192.168.4.1}.
  \item A phone was connected to the \texttt{ESP32\_DHT11} network and the address
        opened in its browser; the page showed the temperature and humidity and
        refreshed itself every two seconds. This was recorded on video.
\end{enumerate}
...
\begin{keybox}
The sensor was read successfully over the single data wire on both boards. In
\textbf{Part A} the Raspberry Pi printed a temperature and humidity pair every two
seconds; over the recorded run of \textbf{18 readings} the temperature fell steadily
from $30.9$ to $28.7\,^{\circ}\mathrm{C}$ while the relative humidity rose from $46$ to
$53\,\%$. Four attempts in the same run failed the checksum or returned an incomplete
frame; each was reported by the program and the next read succeeded. In
\textbf{Part B} the ESP32 served its page at \texttt{192.168.4.1}, and the phone showed
$\mathbf{26.10\,^{\circ}C}$ \textbf{and} $\mathbf{55.80\,\%}$ in the lab.
\end{keybox}
...
\subsection*{Observed output --- Part B, ESP32 web page}
...
\toprule
\rowcolor{soft}
\textbf{Output} & \textbf{Where} & \textbf{Value shown} & \textbf{Remark} \\
\midrule
4 & Phone browser at \texttt{http://192.168.4.1} & $26.10\,^{\circ}\mathrm{C}$, $55.80\,\%$ &
    page titled ``ESP32 DHT11 Sensor'', served by the ESP32 access point \texttt{ESP32\_DHT11}; refreshes every 2\,s \\
5 & Arduino Serial Monitor & \texttt{ESP32 IP address: 192.168.4.1} &
    printed by the sketch after \texttt{Wi-Fi started!} and before \texttt{Web server started!} \\
\bottomrule
\end{tabularx}

\vspace{6pt}
\noindent The ESP32 run took place in the air-conditioned lab on a different day from
Part A, which is why its reading ($26.1\,^{\circ}\mathrm{C}$, $55.8\,\%$) does not match
the Pi's; the DHT11's specified accuracy is $\pm2\,^{\circ}\mathrm{C}$ and $\pm5\,\%$
RH in any case.
...
\vspace{0pt}
\centering
\includegraphics[width=\linewidth]{figures/esp32_phone_page.jpg}\\[5pt]
\includegraphics[width=\linewidth]{figures/esp32_ide_serial.jpg}
\end{minipage}\hfill
\begin{minipage}[t]{0.41\textwidth}
\vspace{0pt}
\centering
\includegraphics[width=\linewidth]{figures/esp32_dht11_board.jpg}
\end{minipage}
\caption{Part B. \emph{Top left:} the page served by the ESP32, opened on a phone at
\texttt{192.168.4.1} --- ``Temperature: 26.10 $^{\circ}$C, Humidity: 55.80 \%''.
\emph{Bottom left:} the Arduino IDE with the sketch's page-building code and, in the
Serial Monitor, the start-up messages \texttt{Wi-Fi started!}, \texttt{Network:
ESP32\_DHT11}, \texttt{ESP32 IP address: 192.168.4.1}, \texttt{Web server started!}.
\emph{Right:} the ESP32 DevKit with the DHT11 module on three jumper wires.}
\label{fig:esp}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Sampling rate.} The two-second delay is not just politeness: the
        ...
        returns stale or invalid data. On the ESP32 the page's two-second refresh
        serves the same purpose, since each page load triggers one read.
  \item \textbf{Serving the data instead of printing it.} The ESP32 has no screen,
        but it has Wi-Fi. Running it as an access point means the phone talks to it
        directly, without any router, at the fixed address \texttt{192.168.4.1}; the web
        page is built as a string on every request, so it is always current. The Arduino
        library signals a failed frame by returning \texttt{NaN} rather than raising an
        exception, which the sketch turns into the ``Sensor Error!'' line instead of
        printing nonsense values.
\end{itemize}
...
was read from a single GPIO pin --- of a Raspberry Pi, which printed a fresh pair of
calibrated temperature and humidity values every two seconds, and of an ESP32, which
served the same two values as a web page over its own Wi-Fi network to a phone. Eighteen consecutive readings showed the sensor
...
general-purpose operating system --- and, with the ESP32, took the first step from
reading a sensor to \emph{publishing} its data over a network, which is what an IoT
node does.
